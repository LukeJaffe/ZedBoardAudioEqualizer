...
#define NUM_PIXELS		(1920*1080*3)

Xuint32 black_buf[NUM_PIXELS];
Xuint32 white_buf[NUM_PIXELS];

//int zed_hdmi_display_init( zed_hdmi_display_t *pDemo )
void zed_hdmi_display_init()
{
	// init bufs
	int i;
	...
	zed_hdmi_display_t *pDemo =  &display;
	int ret;
//	Xuint32 timeout = 100;
//	Xuint32 iterations = 0;

    pDemo->uBaseAddr_IIC_HdmiOut = XPAR_ZED_HDMI_IIC_0_BASEADDR;
    pDemo->uDeviceId_VTC_HdmioGenerator = XPAR_ZED_HDMI_DISPLAY_V_TC_0_DEVICE_ID;
    pDemo->uDeviceId_VDMA_HdmiDisplay = XPAR_ZED_HDMI_DISPLAY_AXI_VDMA_0_DEVICE_ID;
    pDemo->uBaseAddr_MEM_HdmiDisplay = XPAR_DDR_MEM_BASEADDR + 0x10000000;
    pDemo->uNumFrames_HdmiDisplay = XPAR_AXIVDMA_0_NUM_FSTORES;
   ...
   for ( frame = 0; frame < 3; frame++ )
   {
      for ( row = 0; row < pDemo->hdmio_height; row++ )
      {
         for ( col = 0; col < pDemo->hdmio_width; col++ )
         {
            ...
            if (col > band_width*(current_band+1) - col_divider/2 && col < band_width*(current_band+1) + col_divider/2)
            {
            	pixel = BLACK;
            }
            *pStorageMem++ = pixel;
...
//            xil_printf("%u, %u, %u\n", row, col, pixel);
         ...
         //memcpy(pStorageMem+row*pDemo->hdmio_width + frame*pDemo->hdmio_width*pDemo->hdmio_height, row_pix, pDemo->hdmio_width*4);
         //memset(pStorageMem + row*pDemo->hdmio_width + frame*pDemo->hdmio_width*pDemo->hdmio_height, WHITE, pDemo->hdmio_width*4);
      }
      //memset(pStorageMem + 1920*1080*frame, WHITE, 1920*1080*4);
   }

   //memcpy(white_buf,pStorageMem,NUM_PIXELS); //src, dst, num
//   if (heights[3] % 2)
//	   memset(pStorageMem, WHITE, NUM_PIXELS*4);
   	   //memcpy(pStorageMem,white_buf,NUM_PIXELS);
//   else
//	   memset(pStorageMem, BLACK, NUM_PIXELS*4);
   	   //memcpy(pStorageMem,black_buf,NUM_PIXELS);

...
int audioRxTx_start(audioRxTx_t *pThis)
{
	...
	XScuGic_Connect(pGic, XPS_FPGA0_INT_ID, (Xil_ExceptionHandler) audioRxTx_isr, (void*) pThis);

	// enable IRQ interrupt at GIC
	XScuGic_Enable(pGic, XPS_FPGA0_INT_ID);

	// define priority and trigger type for AXI Stream FIFO IRQ
	XScuGic_SetPriorityTriggerType(pGic, XPS_FPGA0_INT_ID, 0xA0, 0x3);
